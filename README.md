...
系统能够从全文公开网（samr.gov.cn）和工标网（csres.com）实时检索、抓取国家标准、行业标准信息，并自动将数据合并保存到本地 SQLite 数据库中。

### 核心功能
1. **双源实时搜索与抓取**：支持输入关键词时，系统自动从权威数据源抓取第一页数据，并与本地数据库无缝合并。
2. **多线程后台批量抓取**：支持在独立的**任务监控中心**开启后台自动翻页批量抓取，并实时查看进度与爬虫日志。
3. **缺失数据自动修补 (Rescrape)**：系统具备数据自愈能力，能自动全盘扫描本地数据库，针对丢失或损坏的记录去权威官网重新补齐全字段。
4. **防封禁智能保护 (Anti-Ban)**：具备全局 IP 拦截监控器，在遭遇 `HTTP 500` 等频率限制时，瞬间熔断所有爬取任务保护 IP，并自带数据回滚机制不让已损坏的信息污染现有库。
5. **智能前缀与定时任务**：系统会自动提取已抓取标准的分类前缀，配合 `node-cron` 定时任务，在每天凌晨 2:00 完成全量静默自动更新。
6. **自动化数据库备份**：在专属的**数据库备份管理**面板内，支持手动或定时完成 `.bak` 多版本安全转储与重置回滚。
7. **响应式设计**：完美适配桌面端和移动端浏览器。
